...
fn run_harness(tmux_tmpdir: &Path, result_file: &Path) {
    let script = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/kill_session_selection.exp");
...
#[test]
fn killing_session_keeps_selection_at_same_tree_row() {
    ...
    assert!(
        ...
        "expected renamed session row, got {}",
        attached.trim()
    );
