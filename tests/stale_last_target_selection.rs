...
fn unique_temp_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    PathBuf::from(format!("/tmp/tt-stale-{}-{nanos}", std::process::id()))
}
...
fn run_harness(tmux_tmpdir: &Path, result_file: &Path) {
    let script =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/stale_last_target_selection.exp");
...
#[test]
fn stale_last_target_prefers_attached_session_row() {
    ...
    let beta_target = tmux(
        &tmux_tmpdir,
        &["display-message", "-p", "-t", "beta", "#{pane_id}"],
    );
    set_last_target(&tmux_tmpdir, &beta_target);
    tmux(&tmux_tmpdir, &["kill-session", "-t", "beta"]);

    run_harness(&tmux_tmpdir, &result_file);

    let attached = fs::read_to_string(&result_file).expect("read result file");
    assert!(
        attached.trim().starts_with("zzz:"),
        "expected attached session row to be renamed, got {}",
        attached.trim()
    );
