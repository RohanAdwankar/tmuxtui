...
fn draw_preview(frame: &mut Frame<'_>, area: Rect, state: &DrawState<'_>) {
    ...
    let status = Paragraph::new(state.preview_status.clone())
        ...
        .wrap(Wrap { trim: false });
    let preview = Paragraph::new(bottom_lines(&state.preview_text, sections[1].height))
...
fn bottom_lines(text: &str, height: u16) -> String {
    let skip = text.lines().count().saturating_sub(height as usize);
...
fn draw_picker(frame: &mut Frame<'_>, area: Rect, state: &DrawState<'_>) {
    ...
    let divider = Block::default()
        ...
        .border_style(separator_style);
    let preview = Paragraph::new(bottom_lines(&state.picker_preview, body[2].height))
        .style(Style::default().bg(Color::Indexed(234)).fg(Color::White));
...
mod tests {
    use super::{
        bottom_lines, marker_column, pane_tree_label, pane_tree_line, session_label, window_tree_label,
    };
    use crate::{
        app::App,
    ...
    fn preview_scrolls_to_bottom_when_text_exceeds_area() {
        let text = "1\n2\n3\n4\n5";
        assert_eq!(bottom_lines(text, 3), "3\n4\n5");
        assert_eq!(bottom_lines(text, 8), text);
    }
