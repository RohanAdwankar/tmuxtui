...
def walk(term: Terminal, renderer: Renderer) -> None:
    ...
    term.send("f")
    term.send("g")
    h(term, "picker: space f g opens the pane picker", 0.9)
    term.text("job-1843")
    h(term, "picker: query searches captured pane text and previews the match", 1.2)
    term.send(b"\r")
    h(term, "picker: enter attaches to the selected pane", 1.0)
    term.send(b"\x11")
    h(term, "detach: ctrl-q returns from the picker target to tmuxtui", 1.0)
    term.send("f")
    term.text("opss")
    term.backspace(1)
