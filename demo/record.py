...
def walk(term: Terminal, renderer: Renderer) -> None:
    ...
    term.send("o")
    h(term, "create: o adds a second pane", 0.8)
    term.send(b"\r")
    h(term, "attach: the window now has two panes", 1.0)
    term.send(b"\x11")
    h(term, "detach: return before changing the layout", 0.7)
    term.send("S")
    h(term, "split: S creates a left/right split", 0.7)
    term.send(b"\r")
    h(term, "attach: the selected pane split appears side by side", 1.0)
    term.send(b"\x11")
    h(term, "detach: return before the next split", 0.7)
    term.send("s")
    h(term, "split: s creates a top/bottom split", 0.7)
    term.send(b"\r")
    h(term, "attach: the selected pane split appears stacked", 1.0)
    term.send(b"\x11")
    h(term, "detach: return before zooming", 0.7)
    term.send("z")
    h(term, "zoom: z toggles pane zoom", 0.7)
    term.send(b"\r")
    h(term, "attach: the selected pane is zoomed", 1.0)
    term.send(b"\x11")
    h(term, "detach: return before restoring layout", 0.7)
    term.send("z")
    h(term, "zoom: z restores the split layout", 0.7)
    term.send(b"\r")
    h(term, "attach: the full split layout is visible again", 1.0)
    term.send(b"\x11")
    h(term, "detach: return to continue editing", 0.7)
    term.send("g")
    term.send("g")
    term.send("r")
    term.backspace()
